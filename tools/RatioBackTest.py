...
def SelectGroupsByRatio_Oneday(ia, date, main_factor_name, LongRatio=None, ShortRatio=None, groupNum=None):
    ...
    # 要被筛除的样本 2next为 na (可能是涨停、停牌) 样本
    todel = np.isnan(ia.factors[date]['2next'])

    # 删除部分na样本
    mask = np.isnan(main_factor) | todel
...
# 按照比例回测一天分组情况
def RatioBackTest_Onday(ia, date, main_factor_name, LongRatio=None, ShortRatio=None, groupNum=None,
                        decay_coefficients=[1], other_fac_to_zip=None):
...
# 对每一天都进行相同回测
def RatioBackTest(ia, main_factor_name, LongRatio, ShortRatio, groupNum=6, other_fac_to_zip=None, start_min=5, end_min=237, start_date = None, end_date = None):
    ZIPRESListDict = {}
