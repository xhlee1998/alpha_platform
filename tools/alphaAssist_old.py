...
def read_features_from_ds(tickers,features,strat_date,end_date,ds_path = None):
    ...
    if ds_path is not None:
        ...
        for YrMth in YrMthList:
            ...
            subdates = ds.get_trade_dates(start_date = start, end_date = end)
            res = ds.get_mins_feature(tickers=tickers, dates=subdates, fields=features, where=reversePathDict[YrMth])
            for d in subdates:
                if d not in output_dict.keys():
                    output_dict[d] = {}
...
def multi(atp, alpha_list, factor_names=None, using_rank=True, neutral_industry=False, neutral_mktcap=False, only_fac=False, thr_dict=None,
    ...
    for date in date_list:
        ...
        if not 'number' in atp.factors[date].keys():
            ...
            vwaptick = int(cfg.vwap_period * 1 * cfg.sample_freq)
            ...
            val = pd.DataFrame(np.where(val < 1e-2, 0, val))
            ...
            vol = pd.DataFrame(np.where(vol < 1e-2, 1, vol))
            vwap = (val / vol).shift(-vwaptick, axis=1)

            #                 vwap = np.where(atp.factors[date]['bp1'] == 0, np.nan,vwap)
            vwap = vwap.replace(0, np.nan)
            vwap = vwap.fillna(mp).values * AdjFactor.loc[atp.tickers, date].values.reshape(-1, 1)
            vwap = pd.DataFrame(vwap).fillna(method='ffill', axis=1)
            atp.factors[date]['vwap_liu'] = vwap.values
            ...
            for key in cfg.apd_rtn.keys():
                for period in cfg.apd_rtn[key]:
                    if 'avg' in key:
                        ...
                        calcrtn = (vwap_liu.diff(int(cfg.order_time * period), axis=1).shift(
                            -int(cfg.order_time * period), axis=1) / vwap_liu)
